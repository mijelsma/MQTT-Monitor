...
class DashboardPanel extends StatelessWidget {
      ...
      children: [
        UiSection(
          ...
          children: [
            ...
            UiSliderRow(label: s.dashboardPanelDotSize, subtitle: s.dashboardPanelDotSizeHint, value: vm.defaultDotSize, min: 0, max: 10, divisions: 20, displayValue: vm.defaultDotSize.toStringAsFixed(1), onChanged: vm.setDefaultDotSize),
            UiSliderRow(
              label: s.dashboardPanelMaxSamples,
              subtitle: s.dashboardPanelMaxSamplesHint,
              value: DashboardSeriesPolicy.settingSliderPosition(vm.defaultMaxSamples),
              min: 0,
              ...
              maximumLabel: DashboardSeriesPolicy.settingsMaximumSamples.toString(),
              semanticFormatterCallback: (position) => DashboardSeriesPolicy.samplesForSettingSlider(position).toString(),
              onChanged: (position) => vm.setDefaultMaxSamples(DashboardSeriesPolicy.samplesForSettingSlider(position)),
            ),
          ],
        ),
        if (layouts.isEmpty)
          ...
          UiSection(
            ...
            sortable: true,
            onReorder: (oldIndex, newIndex) => vm.reorderLayouts(oldIndex, newIndex),
            children: [
                ...
                UiSortableRow(
                  ...
                  leading: Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: AppColors.brokerGradientFor(layouts[index].colorIndex)),
                      borderRadius: BorderRadius.circular(9),
                    ),
                    child: const Icon(Icons.dashboard_rounded, size: 18, color: Colors.white),
                  ),
                  title: layouts[index].title,
                  trailing: [ScopeBadge(isGlobal: layouts[index].isGlobal, brokerCount: layouts[index].brokerIds.length)],
  ...
  Future<void> _openEdit(BuildContext context, DashboardLayoutModel layout) async {
    final vm = context.read<SettingsViewModel>();
