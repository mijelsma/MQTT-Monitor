...
class BrokerRepository extends ChangeNotifier {
  ...
  Future<bool> _commit(List<BrokerEntry> brokers, String? activeId, _CleanupPlan cleanup) async {
  ...
  Future<List<BrokerEntry>> _hydrateCredentials(List<BrokerEntry> brokers) async {
    final hydrated = <BrokerEntry>[];
  ...
  Future<_PreparedCredential> _prepareCredential(BrokerEntry broker, {BrokerEntry? previous}) async {
  ...
  _CleanupPlan _cleanupBetween(BrokerEntry previous, BrokerEntry next) {
  ...
  _CleanupPlan _cleanupFor(BrokerEntry broker) {
  ...
  Set<String> _certificatePaths(BrokerEntry broker) => {
