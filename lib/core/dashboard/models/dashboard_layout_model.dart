...
class DashboardLayoutModel {
  DashboardLayoutModel({required this.id, required this.title, List<String> brokerIds = const [], this.colorIndex = 0, List<GraphCardModel> cards = const []}) : brokerIds = List.unmodifiable(brokerIds), cards = List.unmodifiable(cards);
  ...
  DashboardLayoutModel copyWith({String? title, List<String>? brokerIds, int? colorIndex, List<GraphCardModel>? cards}) {
    return DashboardLayoutModel(id: id, title: title ?? this.title, brokerIds: brokerIds ?? this.brokerIds, colorIndex: colorIndex ?? this.colorIndex, cards: cards ?? this.cards);
  }
  ...
  factory DashboardLayoutModel.fromJson(Map<String, dynamic> json) {
    ...
    return DashboardLayoutModel(id: json['id'] as String, title: json['title'] as String, brokerIds: ids, colorIndex: json['colorIndex'] as int? ?? 0, cards: cards);
  }
