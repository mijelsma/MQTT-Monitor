...
class DashboardRepository extends ChangeNotifier {
  ...
  Future<void> setLayouts(List<DashboardLayout> layouts) async {
    _validateLayouts(layouts);
    final immutable = List<DashboardLayout>.unmodifiable(layouts);
...
List<DashboardLayout> _decodeLayouts(Object? raw) {
  ...
  final layouts = decoded.map((value) => DashboardLayout.fromJson(Map<String, dynamic>.from(value as Map))).toList(growable: false);
  _validateLayouts(layouts);
  return layouts;
}
...
void _validateLayouts(List<DashboardLayout> layouts) {
  final ids = <String>{};
