...
class BrokerEntry {
  ...
  String get effectiveClientId {
    // Use the broker's configured client ID or the default 'mqtt_monitor' if none provided.
    final base = (clientId != null && clientId!.isNotEmpty) ? clientId! : 'mqtt_monitor';

    // If random suffix is disabled, return the base client ID as-is.
    if (!randomClientIdSuffix) return base;

    // Append a random 6-digit hex suffix to the base client ID to ensure uniqueness.
    final hex = Random().nextInt(0xFFFFFF).toRadixString(16).toUpperCase().padLeft(6, '0');

    // Return the combined client ID with suffix.
    return '${base}_$hex';
  }
