...
void main() {
  ...
  SubscriptionEntryModel subscription(String id, String filter, {bool enabled = true, int retention = 10}) => SubscriptionEntryModel(
    id: id,
    topic: filter,
    history: SubscriptionHistoryPolicyModel(enabled: enabled, retention: retention),
  );

  Future<void> addBroker(String id, List<SubscriptionEntryModel> subscriptions, {bool makeActive = true}) {
    return brokers
        .add(
          BrokerEntryModel(id: id, name: id, host: '$id.invalid', subscriptions: subscriptions),
  ...
  test('disabling history preserves existing values and skips new work', () async {
    ...
    await brokers.update(broker.copyWith(subscriptions: [broker.subscriptions.single.copyWith(history: const SubscriptionHistoryPolicyModel(enabled: false, retention: 3))]));
