...
void main() {
  ...
  test(
    'changing a configured channel immediately restarts discovery',
    () async {
      final source = _FakeReleaseSource((_) async => null);
      final service = AppUpdateService(
        preferences: preferences,
  ...
  test(
    'uses the selected archive and delegates check download and restart',
    () async {
      final selection = _selection(
        'v1.2.3-beta.1',
        channel: AppUpdateService.betaChannel,
      );
  ...
  test('a newer discovery epoch prevents stale results from winning', () async {
    ...
    first.complete(
      _selection('v9.0.0', channel: AppUpdateService.stableChannel),
    );
...
GitHubReleaseSelection _selection(String tag, {required String channel}) {
  ...
  final release = GitHubRelease(
    ...
    assets: [
      GitHubReleaseAsset(
        name: GitHubReleaseSource.archiveAssetName,
...
class _FakeReleaseSource implements AppUpdateReleaseSource {
  ...
  final Future<GitHubReleaseSelection?> Function(bool includePrereleases)
  _findLatest;
