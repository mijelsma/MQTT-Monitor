...
void main() {
  ...
  test(
    'updates counts incrementally and only rederives rows for membership changes',
    () {
      source
      ...
      expect(
        workspace.visibleRows.first.metrics.value,
        const TopicNodeMetrics(topicCount: 1, messageCount: 1),
      );
      ...
      expect(
        workspace.visibleRows.first.metrics.value,
        const TopicNodeMetrics(topicCount: 1, messageCount: 2),
      );
      ...
      expect(
        workspace.visibleRows.first.metrics.value,
        const TopicNodeMetrics(topicCount: 2, messageCount: 5),
      );
      ...
      expect(
        workspace.visibleRows.first.metrics.value,
        const TopicNodeMetrics(topicCount: 1, messageCount: 3),
      );
      expect(pulses.paths, ['factory/beta', 'factory/alpha']);
    },
  );
  ...
  test(
    'combined filters keep topic matches incremental and add payload matches',
    () {
      source
      ...
      expect(workspace.visibleRows.map((row) => row.node.fullPath), [
        'factory',
        'factory/needle-topic',
        'factory/other',
      ]);
      expect(
        workspace.visibleRows.first.metrics.value,
        const TopicNodeMetrics(topicCount: 2, messageCount: 4),
      );
...
MQTTMessage _message(String topic, String payload, int second) => MQTTMessage(
  topic: topic,
  payload: payload,
