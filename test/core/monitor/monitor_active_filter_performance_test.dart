...
void main() {
  test('active value filtering stays incremental as topic count grows', () async {
    ...
    const budgets = {
      ...
      100: Duration(milliseconds: 300),
      1000: Duration(milliseconds: 300),
    };
    ...
    for (final entry in budgets.entries) {
      results.add((
        topicTotal: entry.key,
        elapsed: await _profile(
          topicTotal: entry.key,
    ...
    debugPrint(
      'Active value-filter regression guard, $messageTotal existing-topic messages per case:',
    );
    debugPrint('topics | incremental update us | budget us');
    for (final result in results) {
      debugPrint(
        '${result.topicTotal} | ${result.elapsed.inMicroseconds} | ${result.budget.inMicroseconds}',
      );
      expect(
        result.elapsed,
        lessThan(result.budget),
        reason:
            '${result.topicTotal}-topic active filtering exceeded its regression budget.',
      );
    }
  });
}

Future<Duration> _profile({
  required int topicTotal,
  required int messageTotal,
}) async {
  ...
  final ingestion = MessageIngestionCoordinator.fromStream(
    source.stream,
    dependencies.brokers,
  )..initialize();
  final projection = TopicProjection(ingestion, dependencies.brokers)
    ..initialize();
  final history = MessageHistoryService(
    ingestion,
    dependencies.historyPreferences,
    dependencies.brokers,
  );
  ...
  for (var topic = 0; topic < topicTotal; topic++) {
    source.add(
      _message(
        'devices/device-$topic/value',
        topic == 0 ? 'needle-$padding' : 'ordinary-$topic-$padding',
        topic,
      ),
    );
  }
  ...
  final workspace = MonitorWorkspaceController(
    ...
    pulses: _NoopTopicPulseController(),
  );
  workspace.expandAll();
  ...
  for (var sequence = 2; sequence < messageTotal + 2; sequence++) {
    ...
    source.add(
      _message(
        'devices/device-$topic/value',
        topic == 0
            ? 'needle-$sequence-$padding'
            : 'ordinary-$topic-$sequence-$padding',
        sequence,
      ),
    );
  }
  ...
  expect(workspace.visibleRows.map((row) => row.node.fullPath), [
    'devices',
    'devices/device-0',
    'devices/device-0/value',
  ]);
  expect(
    workspace.visibleRows.first.metrics.value.messageCount,
    1 + (messageTotal + 1) ~/ topicTotal,
  );
...
MQTTMessage _message(String topic, String payload, int millisecond) =>
    MQTTMessage(
      topic: topic,
      payload: payload,
      receivedAt: DateTime(2026, 1, 1, 0, 0, 0, millisecond),
    );
