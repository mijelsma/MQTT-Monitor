...
void main() {
  ...
  test('inserts sorted branch topics with incremental counts and one shared value', () {
    source
    ...
    expect(projection.roots.map((node) => node.segment), ['Alpha', 'beta', 'plant']);
    ...
    expect(plant.valueNotifier.value?.receivedAt, DateTime(2026, 1, 1, 0, 0, 1));
    ...
    expect(identical(history.getHistory('plant').single, plant.valueNotifier.value), isTrue);
  });

  test('filters by topic and value while keeping topic-filter counts granular', () {
    source
  ...
  test('value-only traffic updates narrow signals without rebuilding visible rows', () {
  ...
  test('deletion uses topic boundaries, repairs counts, and clears matching history', () {
    source
...
MQTTMessage _message(String topic, String payload, int second, {bool retain = false, int qos = 0}) => MQTTMessage(topic: topic, payload: payload, receivedAt: DateTime(2026, 1, 1, 0, 0, second), retain: retain, qos: qos);
