...
void main() {
  test('validates and resolves before forwarding one publish', () async {
    ...
    final result = await service.execute(
      const PublishCommand(
        topicTemplate: r'sensors/${DEVICE}/set',
  ...
  test('returns precise failures without touching transport', () async {
    ...
    final cases = <({PublishCommand command, PublishCommandFailure failure})>[
      (
        command: const PublishCommand(topicTemplate: ''),
        failure: PublishCommandFailure.emptyTopic,
      ),
      (
        command: const PublishCommand(topicTemplate: r'sensor/${BROKEN'),
        failure: PublishCommandFailure.invalidTemplate,
      ),
      (
        command: const PublishCommand(topicTemplate: r'sensor/${ID}'),
        failure: PublishCommandFailure.missingVariables,
      ),
      (
        command: const PublishCommand(topicTemplate: 'sensor/+'),
        failure: PublishCommandFailure.invalidTopic,
      ),
      (
        command: const PublishCommand(
          topicTemplate: 'sensor/value',
          payload: '{broken',
          payloadIsJson: true,
        ),
        failure: PublishCommandFailure.invalidJson,
      ),
      (
        command: const PublishCommand(topicTemplate: 'sensor/value', qos: 3),
        failure: PublishCommandFailure.invalidQos,
      ),
    ];
  ...
  test(
    'reports offline when validated command has no active transport',
    () async {
      final service = PublishCommandService(
        _RecordingTransport(connected: false),
        const TemplateResolver(),
      );

      final result = await service.execute(
        const PublishCommand(topicTemplate: 'sensor/value'),
        const {},
      );
