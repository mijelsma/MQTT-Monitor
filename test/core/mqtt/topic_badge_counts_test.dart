...
TopicTreeNode addMessage(
  ...
  for (final segment in segments) {
    ...
    node = level.putIfAbsent(
      segment,
      () => TopicTreeNode(segment: segment, fullPath: fullPath),
    );
    level = node.children;
  }
...
void main() {
  test('filtered parent counts are stable and do not double count', () {
    final roots = <String, TopicTreeNode>{};
    ...
    TopicBadgeCounts calculate() => deriveTopicBadgeCounts(
      roots.values,
      includesTopic: (node) => node.fullPath.contains('/0001/'),
    )[parent]!;
  ...
  test('rapid child arrivals update a filtered parent exactly once each', () {
    ...
    for (var expected = 1; expected <= 50; expected++) {
      if (expected > 1) {
        ...
        leaf.valueNotifier.value = TopicNodeValue(
          payload: 'target event $expected',
          seq: previous.seq + 1,
          receivedAt: DateTime(2026, 1, 1, 0, 0, expected),
        );
      }
      final first = deriveTopicBadgeCounts(
        roots.values,
        includesTopic: (node) =>
            node.valueNotifier.value?.payload.contains('target') ?? false,
      )[parent]!;
      final repeated = deriveTopicBadgeCounts(
        roots.values,
        includesTopic: (node) =>
            node.valueNotifier.value?.payload.contains('target') ?? false,
      )[parent]!;
