...
import 'package:mqtt_monitor/core/mqtt/certificate_file_access.dart';
...
void main() {
  group('certificate validation', () {
    ...
    test('requires client key + cert (Root CA is optional)', () async {
      await expectLater(service.validateConfiguration(const ClientCertificateConfig(rootCaPath: '/certs/ca.pem')), throwsA(isA<CertificateValidationException>()));
      await expectLater(service.validateConfiguration(const ClientCertificateConfig()), completes);
    });
  });
  ...
  test('builds a SecurityContext from mocked stored paths', () async {
    ...
    final context = await service.buildSecurityContext(const ClientCertificateConfig(rootCaPath: '/private/ca.pem', clientPrivateKeyPath: '/private/key.pem', clientCertificatePath: '/private/cert.pem'));
  ...
  test('builds a SecurityContext with only client key + cert (no Root CA)', () async {
    ...
    final context = await service.buildSecurityContext(const ClientCertificateConfig(clientPrivateKeyPath: '/private/key.pem', clientCertificatePath: '/private/cert.pem'));
